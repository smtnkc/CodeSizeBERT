...
def extract_info_from_filename(filename):
    # Example: code_cosmic-shuffled-data-predby-codebert-code_cosmic.csv
    parts = filename.replace('.csv', '').split('-')
    test_data = parts[0]  # code_cosmic
    model = parts[4]  # codebert
    train_data = parts[5]  # code_cosmic
...
def main():
    ...
    for filename in os.listdir('preds'):
        if not filename.endswith('.csv') or 'codebert' not in filename:
            continue
        ...
        for feature in features:
            # pass if the feature is not in the DataFrame
            if feature not in df.columns or f'{feature}_pred' not in df.columns:
                continue
    ...
    for metric_name, rows in metrics_dfs.items():
        ...
        # Save to CSV
        df_pivot.to_csv(f'stats_for_code/{metric_name}.csv', index=False)
