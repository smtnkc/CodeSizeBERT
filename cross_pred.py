...
out_model_name  = f"{in_model_name}-{dataset}" # codebert-code_cosmic
output_csv      = f"preds/{dataset}-shuffled-data-predby-{out_model_name}.csv" # code_cosmic-shuffled-data-predby-codebert-code_cosmic.csv
log_path        = f"logs/{dataset}-{target}-predby-{out_model_name}.csv" # Changed extension to .csv
out_model_path  = f"checkpoints/{out_model_name}-{target}.pt" # codebert-code_cosmic-entry.pt

# Initialize log DataFrame
log_df = pd.DataFrame(columns=['Fold', 'Epoch', 'Loss', 'MSE', 'MAE', 'NMAE', 'ACC', 'MMRE', 'PRED30', 'NonZeroPerc'])
...
for fold in range(5):
    ...
    train_ds = TextRegressionDataset(
        train_df[input_col], train_df[target], tokenizer
    )
    test_ds  = TextRegressionDataset(
        test_df[input_col],  test_df[target],  tokenizer
    )
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        base_model,
        num_labels=1,
        problem_type="regression" # tells HF to use MSELoss under the hood
    ).to(device)
    ...
    for epoch in range(1, 6):
        ...
        if epoch == 1 and fold == 0:  # Print header on first iteration
            print("\n{:<5} {:<5} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format(
                "Fold", "Epoch", "Loss", "MSE", "MAE", "NMAE", "MMRE", "PRED(30)", "ACC", "NonZero%"
            ))
            print("-" * 100)  # Separator line
        
        print("{:<5} {:<5} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.1f}".format(
            fold+1, epoch, avg_train_loss, mse_epoch, mae_epoch, nmae_epoch, mmre, pred30, exact_match_acc, nonzero_perc
        ))
    ...
    print("-" * 100)  # Separator line
    # Keep predictions from best epoch
